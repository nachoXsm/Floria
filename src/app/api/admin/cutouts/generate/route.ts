...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      // Cuota diaria agotada: mensaje claro (se resetea a la medianoche UTC).
      if (/neuron|allocation|limit|429/i.test(detail) || res.status === 429) {
        return NextResponse.json({
          error: 'Cloudflare sin cuota diaria — probá de nuevo mañana (se resetea a la medianoche UTC)',
          quota: true,
        }, { status: 429 })
      }
      return NextResponse.json({ error: 'Cloudflare', detail }, { status: 502 })
    }
    const data = await res.json()
    const image = data?.result?.image
    if (!image) return NextResponse.json({ error: 'Sin imagen de Cloudflare' }, { status: 502 })
    return NextResponse.json({ image, source: 'cloudflare' }) // base64 JPEG
  } catch (e) {
